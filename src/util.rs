...
use std::thread;
use time;
use block::{SignetBlock, SignetBlockHeader};
...
impl<'a> From<&'a SignetBlock> for BlockMeta {
    fn from(block: &'a SignetBlock) -> BlockMeta {
        BlockMeta {
...
fn to_bitcoin_network (network: &Network) -> BNetwork {
    match network {
        ...
        Network::Signet => BNetwork::Regtest, // @FIXME
    }
}
...
fn to_bech_network (network: &Network) -> B32Network {
    match network {
        ...
        Network::Signet => B32Network::Regtest, // @FIXME
    }
}
